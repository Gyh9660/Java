...
/* MS-SQL, MYSQL은 자동 커밋 트랜잭션 : DML (insert, update, delete) :commit이 자동으로 처리
	
	begin transaction;
	insrt ~~~~;
	commit;

	MSSQL ,MySQL 은 자동 커밋이 되므로, update, delete 구문을 사용할때
	명시적으로 트랜잭션을 시작, commit or rollback 해줘야한다

*/

select * from employee;
select eno 사원번호 , ename as 사원명, job, job, salary, salary from employee;
