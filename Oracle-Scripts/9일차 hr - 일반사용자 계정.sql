...
/*
    index는 주기적으로 REBUILD 해줘야한다., (1주일 , 1달~)
        index page는 조각난다 (insert , update , delete) 빈번하게 일어나면
*/

--index rebuild 를 해야하는 정보 얻기 : index의 tree 깊이가 4이상인 경우 조회가 되면 rebuild할 필요가 있다.
...
/*
    index를 사용해야 하는 경우 
        1.  테이블의 행(row, 레코드)의갯수가 많은경우 
        2.  where 절에서 자주사용되는 컬럼.
        3. join 시 사용되는 키 컬럼.
        4. 검색결과가 원본테이블 데이터의 2%~4%정도 되는 경우
        5. 해당 컬럼이 null이 포함하는 경우 (색인은 null 제외)

    index를 사용하면 안좋은 경우
        1. 테이블의 행의 갯수가 적은 경우.
        2. 검색 결과가 원본테이블의 많은비중을 차지하는 경우
...
/*
    index 종류
        1. 고유 인덱스 (unique index) : 컬럼의 중복되지 않는 고유한 값을 갖는 인덱스 (primary key, unique(
        2. 단일 인덱스 (single index) : 한컬럼에 부여되는 인덱스
        3. 결합 인덱스 (composite index) : 여러컬럼을 묶어서 생성한 인덱스
        4. 함수 인덱스 (Function base index) : 함수를 적용한 컬럼에 생성한 인덱스
...
/*
    사용권한 : DBMS는 여러명이 사용한다  
    -- 각 사용자별로 계정을 생성: DBMS에 접속할 수 있는 사용자를 생성.
            (인증 (Authentication : Credential (identity + Password) 확인
            (허가 (Authorization : 인증된 사용자에게 Oracle의 시스템권한,객체 (테이블,뷰,트리거,함수)권한
                -System privileges : 오라클 전체적으로 권한부여 (오라클의 전반적인 권한 할당)
                -Object privileges : 객체(테이블, 뷰,트리거,함수,저장프로시저,시퀀스,인덱스) 접근권한
                
*/

-- Oracle에서 계정생성. (일반 계정에서는 계정을 생성할 수 있는 권한이 없다.)
show user;
