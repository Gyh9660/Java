...
public class ProListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        int totalCount = pdao.selectCount(map);  // 게시물 개수 (게시물에서 start , end)
	...
	// 게시물 목록을 받아오기 (DAO 객체에 작업을 전달 )
        ...
        List<ProDTO> boardLists = pdao.selectListPage(map);  // 게시물 목록 받기
        pdao.close(); // DB 연결 닫기
       /*
	  ...
	  //View페이지로 변수의 값을 전달하기위해서
	    ...
	    req.getRequestDispatcher("/shop/prolist.jsp").forward(req, resp);
