...
public class EditController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		//get 요청일때 처리
		//get방식으로 요청을 받아서 클라이언트가 넘기는 변수를 할당받아서 view페이지로 넘김.
		//비밀번호 검증후 검증이 완료되면 idx값을 받아서 edit.jsp페이지로 이동
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String saveDirectory = req.getServletContext().getRealPath("/Uploads");
			...
			//업로드 할 파일의 최대 용량 확인 (web.xml의 설정을 가져옴 :1MB)
		...
		//2. 파일 업로드 외 처리============================
				//Request 객체가 아니라 MultipartRequest객체에서 Form의 변수값을 받는다.
				//업로드 라이브러리 마다 Form의 변수값을 받는 메소드 이름이 다를 수 있다.
			...
			*/
			...
			//dto 객체의 Ofile, Sfile 은 업로드 경로에 해당파일명이 존재하는경우 처리
			
			//원본 파일명과 저장될 파일 이름 설정
			String fileName = mr.getFilesystemName("ofile");
			if(fileName != null) { //첨무파일이 Uploads 폴더에 존재하는 경우 파일이름을 수정해서 저장
				//새로운 파일명 생성
				String now = new SimpleDateFormat("yyyyMMdd_HmsS").format(new Date());//날짜
				String ext = fileName.substring(fileName.lastIndexOf("."));
				//.을기준으로 뒤에있는 확장자만 게더링해서 가져옴
				String newFileName = now +ext;
				
				System.out.println("now : "+now);
				System.out.println("ext : "+ext);
				System.out.println("1newFileName "+newFileName);
				...
				dto.setSfile(newFileName); //새로운 파일이름 (서버에 저장될 파일 이름)
			...
			MVCBoardDAO dao = new MVCBoardDAO();
			int result = dao.updatePost(dto); //result가 1이라면 업데이트 성공 ,0이면 실패
