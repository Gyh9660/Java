...
public class WriteController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//Get 방식 요청 처리
		//List.jsp (view 페이지) 에서 글쓰기를 클릭했을때 글쓰기 view페이지로 이동 (Write.jsp)
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// Post방식
		//Write.jsp 페이지에서 전송을 클릭했을때 처리 form에서 넘어오는 변수의 값을 처리
		...
		String fileName = mr.getFilesystemName("ofile"); //client의 첨부파일의 물리적인주소
			System.out.println(fileName ); //주석 처리
		...
		if (fileName != null) { //첨부 파일이 비어 있지 않다면
			...
			File newFile = new File(saveDirectory + File.separator + newFileName);
				System.out.println("oldFile : " + oldFile); //주석처리
				System.out.println("newFile : " + newFile); //주석처리
			
			oldFile.renameTo(newFile);
			
			//DTO에 Settor 주입 (조건 : 파일을 업로드 한 경우에만)
			dto.setOfile(fileName); //원래 파일이름
			dto.setSfile(newFileName); // 서버에 저장될  파윌이름
