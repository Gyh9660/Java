...
public class ListController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (searchWord != null) { //검색어가 존재 한다면 map에 값을 저장
			map.put("searchField", searchField);
			map.put("searchWord", searchWord);
		}
		//게시물 갯수 알아오기 (DAO에 selectCount(map)를 던져서 게시물 갯수를 알아옴)
		
		int totalCount = dao.selectCount(map);
		
	//	System.out.println("전체 레코드 수 : "+totalCount);
	//	System.out.println("List.do 요청 시 Controller 페이지 잘 응답.");
	...
	//	System.out.println(blockPage);
		...
		if (pageTemp != null && !pageTemp.equals(" ")) {
			pageNum = Integer.parseInt(pageTemp); // 값이 비어있지 않을때 넘어온 페이지 변수를 정수로 변환해서 저장
		}
	...
	// 게시물 목록을 받아오기 (DAO 객체에 작업을 전달)
		...
		req.getRequestDispatcher("/mvcboared/list.jsp").forward(req, resp);
	}
