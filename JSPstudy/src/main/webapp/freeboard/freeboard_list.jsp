...
<CENTER>
 ...
 <%  //vector : 멀티스레드 환경에서 사용, 모든 메소드가 동기화 처리되어 있다.
  ...
  Vector step=new Vector();		//DB의 step컬럼만 저장하는 벡터
 ...
 try {
  ...
  sql = sql + " masterid desc, replaynum, step, id" ; //중요한 쿼리?
 ...
 // if (true) return; //프로그램 종료
  ...
  if (!(rs.next()))  {
   ...
   do {
    ...
    keyid.addElement(new Integer(rs.getInt("id"))); 
    	//rs에 있는 값 (id컬럼)을 가져와서  vactor에 저장
  ...
  // out.println("현재 페이지 : " + where + "<p>");
   ...
   totalgroup = (totalpages-1)/maxpages +1; //페이징 그룹핑
   
  // out.println("==========maxrows : 3일때 ===========" + "<p>");
  // out.println(" totalgroup (토탈 페이지 그룹) : " + totalgroup + "<p>");
   ...
   for(int j=startrow;j<=endrow;j++) {
    ...
    if ((temp == null) || (temp.equals("")) )  //메일주소가 비어있을때
    ...
    } 						// \는 "특수문자 자체를 출력할때 사용 ↑ = "bgColor = '#DFEDFF'"
    out.println("<TD align=center>");
    out.println(id+"</TD>");
    out.println("<TD>");
    
    //stepi 글의 깊이 , 0 = 처음글 , 1= 답변, 2= 답변의 답변 .....
    int stepi= ((Integer)step.elementAt(j)).intValue();
    int imgcount = j-startrow; 
    
    
    if (stepi > 0 ) { //stepi > 0 => 답변글인 경우
