...
<% request.setCharacterEncoding("EUC-KR"); %> <!-- g한글 처리 -->
...
<%
	...
	//out.println (name + "<p>");
	//out.println (email + "<p>");
	//if (true) return;  //프로그램 을 여기까지만 실행(여기서 멈춤)
	...
	try{
		...
		if(!(rs.next())){  //값이 존재하지 않는 경우
			...
			//out.println("데이터 베이스에 값이 존재 합니다.");
			String em = rs.getString("email");
			//String cont = rs.getString("content");
			if ((em !=null)&&(!(em.equals("")))){ //DB의 email컬럼의 값이 존재하면
			...
			//out.println (cont);
			//서블릿으로 출력, 서블릿 : JAVA에서 웹페이지를 출력 할 수 있는 Java 페이지
